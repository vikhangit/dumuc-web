...
export default function ModalChangeEmail({ visible, onCancel }) {
  const [user] = useAuthState(auth);
  const [loading, setLoading] = useState(false);
  const [usingUser, setUsingUser] = useState();
  const [email, setEmail] = useState("");
  const [emailError, setEmailError] = useState("");
  const [verifyBeforeUpdateEmail, updating1, error1] =
  ...
  const updateEmailNew = useCallback(async () => {
    setLoading(true);
    ...
    reauthenticateWithPopup(user, providerGoogle)
      .then(async () => {

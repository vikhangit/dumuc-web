...
import { MdOutlineArrowForwardIos, MdOutlineSettingsAccessibility } from "react-icons/md";
    ...
    return (
      <main className="w-full">
        ...
        <div className='flex items-center bg-[#C82027] shadow-md shadow-gray-400 w-full px-[15px] py-[20px] gap-x-3'>
                <button onClick={() => router.back()}><FaArrowLeftLong size={24} color="#fff" /></button>
          ...
          <div className="">
            ...
            <div className="flex items-center px-4 mt-2">
                ...
                <div className="relative border border-[#E0D2D2] rounded-md w-[calc(100%-105px)]">
                    <input className="border-0 outline-0 shadow-0 focus:border-0 focus:outline-0 focus:shadow-0 block w-full h-full py-2 pr-[45px] text-center"
                    value={(30 * (value / 100)).toFixed(1)}
                     />
                    <span className="absolute right-0 top-0 w-[38px] h-full rounded-br-md rounded-tr-md bg-[#389FFD4D] flex justify-center items-center">Km</span>
                </div>
            </div>
            <div className="flex justify-end px-4 my-2">
                <div className="w-[calc(100%-105px)]">
                <RangeSlider theme={theme} size={"md"} value={value} onChange={(e) => setValue(e.target.value)} />
                </div>
            </div>
            <div>
            </div>
          </div>
          <div className="rounded-3xl mt-6 px-4 sm:px-8 bg-white mb-3 border-b border-gray-300" style={{
           ...
           {
            loadData ? <div className="h-20 w-full flex justify-center items-center">
              <Spinner /> <div className="ml-2">Đang tìm kiếm</div>
            </div> :
            Array(5).fill().map((x, index) => {
              return <div  key={index} class={`flex justify-between items-center sm:gap-x-4 py-3 ${index !==0  ? "border-t" : "border-0"} border-[#389FFD]`}>
              <Link
            href={``}
          >
            <div class="flex items-center gap-x-2">
              ...
              <div className="md:flex-1 md:min-w-0 text-sm font-medium text-gray-900">
                <div className="text-base font-medium text-[#363535]">Tran Vi Khang</div>
                <div className={`flex gap-x-4 ${sizes.width > 335 ? "flex-row" : "flex-col gap-y-1"}`}>
                <div className="text-xs sm:text-sm text-[#747272]">Ngày tham gia: <span className=''>12/08/2023</span></div>
                <div className="text-xs sm:text-sm text-[#747272]">
                    Khoảng cách: <span className='text-[#FC1717B2]'>1.2km</span>
                  </div>
                </div>
              </div>

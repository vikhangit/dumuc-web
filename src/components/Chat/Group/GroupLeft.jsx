        ...
        {groupList?.length > 0 ? (
          groupList?.map((item, i) => {
            const author = authors?.find(
              (x) =>
                x?.authorId ===
                item?.messages[item?.messages.length - 1]?.formAuthor?.authorId
            );
            if (item.id === search.get("groupId")) {
              if (item?.member?.find((x) => x?.user === user?.uid)) {
                const aList = item?.avatar?.filter(
                  (ele, ind) =>
                    ind ===
                    item?.avatar?.findIndex(
                      (elem) => elem.id === ele.id && elem.avatar === ele.avatar
                    )
                );
                return (
                  <div
                  ...
                  >
                    <div className="w-[45px] h-[45px] rounded-full flex justify-center items-center border border-gray-400">
                      {item?.id === search.get("groupId")
                        ...
                        : aList &&
                          aList?.length > 0 &&
                          aList[0]?.avatar?.length > 0 && (
                            <Image
                              src={
                                item?.avatar[0]?.avatar?.length > 0
                                  ? item?.avatar[0]?.avatar
                                  : "/dumuc/avatar.png"
                              }
                              width={0}
                              height={0}

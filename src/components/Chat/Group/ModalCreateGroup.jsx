...
export default function ModalCreateGroup({ visible, onCancel, onCallback, authors}) {
  ...
  const save =async () => {
    ...
    }).then(async (data) =>   await addDoc(collection(db, "chat-groups", `${data?.id}`, "member"), {
      user: user?.uid,
      createdAt: serverTimestamp(),
      createdBy: user?.uid
    }))
  ...
  return (
    ...
    >
      <div className="mt-8 flex items-center gap-x-2">
        {
          avatar.length > 0 ? <Image src={avatar} width={0} height={0} sizes="100vw" className="w-12 h-12 rounded-full object-cover"  />
          : <button 
          onClick={() => refAvatar.current?.click()}
          className="w-14 h-12 flex justify-center items-center border border-gray-200 rounded-full" >
          {
            loadingAvatar ? <Spinner className="w-4 h-4" /> : <FaCamera color="#999" />
          }
        </button>
        }
        ...
        }} placeholder="Nhập tên nhóm" aria-label="name" className={
      ...
      <input aria-label="avatar" accept="image/*" className='hidden' type='file' ref={refAvatar}  onChange={(e) => handleChangeIamge(e)} />
    </Modal>

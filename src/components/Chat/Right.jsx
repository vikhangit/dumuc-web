...
import { RiFileVideoFill, RiFolderVideoLine } from 'react-icons/ri';
...
export default function ChatRight({userRecieved, setUserRecieved, mobile, setMobile, messages, authors}) {
    ...
    useEffect(() => {
      setMyMessage([])
      const chat2 = messages?.filter( item => item?.member?.find(x => x?.userId === user?.uid))
      const chat = chat2?.find( item => item?.member?.find(x => x?.userId === userRecieved?.userId))
      if(chat){
        setMyMessage(chat?.messages)
      }
     },[messages, userRecieved])
        ...
        array.map(async (x) => 
         ...
         await uploadImage(x, user?.accessToken).then(async (data) => {
              ...
              if(newPhoto.length === array.length){
                ...
                const findChat2 = messages?.filter( item => item?.member?.find(x => x?.userId === user?.uid))
                const findChat = findChat2?.find( item => item?.member?.find(x => x?.userId === userRecieved?.userId))
                if(findChat){
                ...
                }else 
                {
                  await addDoc(collection(db, "chat-rooms"), {
                    ...
                    createdAt: serverTimestamp(),
                  }).then(async (data) =>   await addDoc(collection(db, "chat-rooms", `${data?.id}`, "messages"), {
                      text: newMessage,
                      photos: newPhoto,
                      files: [],
                      createdAt: serverTimestamp(),
  ...
  return (
    <div className={`h-full ${sizes.width > 992 ? "basis-2/3" : `${mobile ? "basis-full" :"hidden"}`}`}>
      <div className='h-[75px] flex justify-between items-center px-[15px] pl-[0px] sm:px-[20px] shadow-md shadow-gray-400'>
          ...
          <Image src={ userRecieved && userRecieved?.user?.photo && userRecieved?.user?.photo?.length > 0 ? userRecieved?.user?.photo : "/dumuc/avatar.png" } width={0} height={0} sizes='100vw' className='w-[45px] h-[45px] rounded-full' />
            <div>
          <Link href="" className='text-base font-semibold'>{userRecieved?.name}</Link>
              ...
              <ul>
                {
                  myMessage?.filter( (ele, ind) => ind === myMessage?.findIndex( elem => elem.id === ele.id && elem.text === ele.text))?.map((item, index) => {
                    ...
                    return  <div className={`flex gap-x-2 sm:gap-x-4 ${item?.formAuthor?.userId === user?.uid ? "flex-row-reverse" : "flex-row"}`}>
        ...
        <ModalViewFile openImage={viewFile} setOpenImage={setViewFile} file={activeFile} />
    </div>
  )
}

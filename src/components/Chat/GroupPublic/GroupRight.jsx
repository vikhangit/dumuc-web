        ...
        {user && groupTo && (
          <>
            ...
            <div className="flex items-center gap-x-3 pr-0 sm:pr-5">
              {groupTo?.member?.find((x) => x?.user === user?.uid) && (
                <IoMdPersonAdd
                  color="#0084ff"
                  size={24}
              ...
              <button className="group relative">
                ...
                <div className="absolute z-[99999] hidden group-hover:flex flex-col justify-start items-start top-full right-0 bg-white shadow-sm shadow-gray-500 text-[10px] sm:text-xs font-medium w-[165px] rounded p-1">
                  <Link
                    href={``}
                    onClick={(e) => {
                      e.preventDefault();
                  ...
                  {groupTo?.leader === user?.uid && (
                    <Link
                      ...
                      className={`hover:bg-[#c80000] hover:text-white w-full rounded px-1.5 py-0.5 text-left flex item-cemter justify-between`}
                    >
                      Yêu cầu than gia nhóm
      ...
      {user && groupTo ? (
        groupTo?.member?.find((x) => x?.user === user?.uid) ? (
          ...
          <div className="w-full h-full flex justify-center items-center flex-col gap-2">
            {groupTo?.requestList?.find((x) => x?.user === user?.uid) ? (

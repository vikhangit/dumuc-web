    ...
    <div id="comments">
      <div className={`flex justify-between ${!focusComment && !qoute && qoute?.length > 0 && "items-center"}`}>
        ...
        <div className={`flex flex-col cursor-text w-[calc(100%-40px)] border border-gray-300 rounded-lg p-2.5 bg-gray-100 ${!focusComment && "flex"}`}>
          ...
          <textarea 
            value={body}
            onClick={() => user ? setFocusComment(true) : router.push(`/auth?url_return=${url_return}`)}
            onChange={e => {
          ...
          <div className="flex justify-end gap-2">
            <label className='cursor-pointer' onClick={() =>  user ? refImage.current.click() :  router.push(`/auth?url_return=${url_return}`)}>

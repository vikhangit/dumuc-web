          ...
          <div className="flex w-full">
            <article className="bg-white py-4 px-4 xl:pr-0 m-auto w-full">
              ...
              <div className="shadow-md shadow-gray-500 pb-2 px-4">
                ...
                <nav
                  className="flex flex-col md:flex-row mt-4 gap-2"
                ...
                >
                  {
                    <Link
                      href={
                        user
                          ? "/forum/post"
                          : `/auth?url_return=${process.env.NEXT_PUBLIC_HOMEPAGE_URL}/forum/post`
                      }
                      className="bg-[#c80000] text-white flex pl-2 pr-3 py-0.5 rounded-md md:rounded-e-md items-center text-sm gap-1 w-fit mx-auto md:mx-0"
                    >
                  ...
                  <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    <li class="inline-flex items-center text-[#828181]">
                      <a
                        href="/forum"
                        class="inline-flex gap-x-2 items-center text-xs sm:text-sm font-medium text-[#828181] hover:text-blue-600 breadcrumb-mobile"
                      >
                        <svg
                    ...
                    <li>
                      <div class="flex items-center ">
                        ...
                        <a
                          href={`/forum/topic/${post?.categoryObj?.slug}/${post?.categoryObj?.categoryId}`}
                          class="ml-1 text-xs sm:text-sm font-medium text-[#828181] hover:text-blue-600 md:ml-2 breadcrumb-mobile"
                        >
                          {post?.categoryParentObj?.name}
                        </a>
                      </div>
                    </li>
                    <li aria-current="page">
                      <div class="flex items-center">
                        ...
                        <Link
                          href={`/forum/topic/${post?.categoryObj?.slug}/${post?.categoryObj?.categoryId}`}
                          class="ml-1 text-xs sm:text-sm font-medium text-[#828181] md:ml-2 dark:text-gray-400 breadcrumb-mobile"
                        >
              ...
              <div className="px-2 sm:px-4 md:px-8 mt-4 shadow-md shadow-gray-500 rounded py-4">
                <div className="">
                  <div className="flex items-center">
                    ...
                    {post?.label && Object.keys(post?.label).length > 0 && (
                      <span
                        ...
                        class={`bg-indigo-100 text-indigo-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded text-center w-fit`}
                      >
                        {post?.label?.name}
                      </span>
                    )}
                  ...
                  <h1
                    className={`my-2 text-xl font-semibold sm:font-bold leading-tight text-[#4A4949] capitalizie`}
                  >
                    {post?.title}
                  </h1>
                </div>
                ...
                />
                <TableOfContent
                  headers={post?.body?.blocks.filter(
                    (x) => x.type === "header"
                  )}
                  onShowScroll={() => setShowScroll(true)}
                />
                <div className="text-xl font-normal text-justify">
                  <EditorjsRender item={post} />
                </div>
                ...
                {showScroll && (
                  <Scroll.Link
                    className="fixed cursor-pointer flex justify-center items-center bottom-[30px] left-[50px] rounded-full z-50 bg-white shadow-lg shadow-gray-600 w-10 h-10"
                    spy={true}
                    smooth={true}
                    offset={-90}
                    to="tab-content"
                ...
                <ArticleComments post={post} user={user} />
              ...
              {post?.relatedPosts?.length > 0 && (
                <div
                  ...
                  style={{
                    backgroundImage:
                      "linear-gradient(to bottom, white, #F7F7F7)",
                  }}
                >
                  <ArticleItems
                    data={{

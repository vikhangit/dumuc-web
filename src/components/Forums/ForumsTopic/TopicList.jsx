  ...
  useEffect(() => {
    ...
    getCategories().then((data) => {
      ...
      cate?.categoryParentId === ""
        ? (payload["categoryParent"] = cate?.categoryId)
        : (payload["category"] = cate?.categoryId);
    ...
    <main className="w-full">
      <div className="w-full">
          ...
          >
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
              ...
              <li>
                <div class="flex items-center ">
                  ...
                  >
                    {
                      categories?.find(
                        (x) => x.categoryId === category?.categoryParentId
                      )?.name
                    }
                  </a>
                </div>
              ...
              <li aria-current="page">
                <div class="flex items-center">
                  <span className="font-semibold">{">>"}</span>
                  <span class="ml-1 text-xs sm:text-sm font-medium text-[#828181] md:ml-2 dark:text-gray-400 breadcrumb-mobile">
                    {category?.name}
                  </span>
                </div>
              </li>
        ...
        <div
          ...
          class="w-full sm:w-[96%] mx-auto mt-3 font-medium text-gray-900 bg-white border border-gray-200 rounded-lg"
        ></div>
        {!initResults || initResults?.length === 0 ? (
          <p className="text-sm sm:text-base font-normal mt-4 text-center">
            Hiện tại chưa có bài viết về danh mục này
          </p>
        ) : (
          <ArticleForumItems
            data={initResults}

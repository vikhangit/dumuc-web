...
export default function ArticeForumChild({ item }) {
  ...
  const expert = item?.body?.blocks?.filter((x) => x.type === "paragraph")[0]
    ?.data?.text;
  return (
    <div
      key={item?.postId}
            ...
            <div className="flex xl:items-center flex-col xl:flex-row justify-start">
              {item?.label && Object.keys(item?.label).length > 0 && (
                <span
                  ...
                  class="bg-white shadow-md shadow-gray-500 text-indigo-800 text-sm font-medium mr-2 px-2.5 py-0.5 rounded text-center w-fit"
                >
                  {item?.label?.name}
                </span>
              )}
              <Link
                href={url}
                className="font-bold text-xl hover:underline text-[#605F5F]"
              >
                {item?.title?.charAt(0).toUpperCase()}
                {item?.title?.slice(1, item?.title?.length).toLowerCase()}
              </Link>
            </div>

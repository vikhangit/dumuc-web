...
export default function ModalImageZoomFeed({openImage, setOpenImage, imageList, index, type="image"}) {
    ...
    style={{
        ...
        <div className={`modal-image w-full sm:w-5/6 md:w-3/4 xl:w-1/2 ${sizes.height > 450 ? "h-full sm:h-5/6 xl:h-3/4" : "h-full"}  mx-auto`}>
            <div className='w-full h-full'>
                ...
                >
                    {
                        imageList?.map((x, index) => <SwiperSlide key={index}>
                            {
                                x.type === "video"
                                ? <video className='w-full h-full object-contain' autoPlay controls loop>

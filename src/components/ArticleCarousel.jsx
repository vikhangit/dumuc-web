    ...
    >
      <div class="px-4 mx-auto max-w-screen-xl">
        <div className="flex justify-between">
          ...
          {moreAction && <div className="mr-1">Xem tất cả</div>}
        </div>
          ...
          >
            {items?.map((item, index) => {
              ...
              const photo = item?.photo
                ? item?.photo
                : photos[0]?.data?.file?.url;
              return (
                <article
                  className={`w-[${width}px] flex-shrink-0 rounded-lg bg-white hover:scale-110 transition-all mt-[20px] cursor-pointer`}
                  key={index}
                >
                  {photo && (
                    <a href={url}>
                      <Image
                        ...
                        sizes="100vw"
                        src={photo}
                        className="mb-2 rounded-t-lg w-full h-32"
                        alt={item?.title}
                      />
                    </a>
                  )}
                    ...
                    <div className="flex">
                      ...
                      <Dropdown
                        ...
                        renderTrigger={() => (
                          <span>
                            <svg
                              class="w-5 h-5 text-gray-900 mx-1"
                              aria-hidden="true"
                              xmlns="http://www.w3.org/2000/svg"
                              fill="currentColor"
                              viewBox="0 0 4 15"
                      ...
                      >
                        <Dropdown.Item>
                          <div class="block px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                            Chặn người dùng
                          </div>
                        </Dropdown.Item>
                      </Dropdown>
                  ...
                  <a
                    href={url}
                    className="m-2 line-clamp-2 text-base font-semibold leading-tight text-gray-900"
                  >
                    {item?.title}
                  </a>
                  <div className="flex my-2 mx-2">
                    ...
                    <li className="mt-0.5 text-xs font-normal text-gray-500 ml-0 sm:ml-2 list-none sm:list-disc">
                      {moment(item.publishDate).format("MMMM DD")}
                    </li>
                  </div>
                </article>
              );

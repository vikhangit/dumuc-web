...
const SearchForm = () => {
  ...
  const onSearchSuggestion = (searchText) => {
    ...
    typingTimeoutRef.current = setTimeout(() => {
      axios(
        `https://us-central1-appfunnel-361810.cloudfunctions.net/api/vieclamphothong/suggestion/${convertViToEn(
          searchText
        )}`,
        {
          method: "GET",
  ...
  const submitSearch = (keyword, category) => {
    ...
    if (keyword) {
      url = `${url}${isPrefix === false ? "" : "&"}description=${keyword}`;
      isPrefix = true;
    }
  ...
  const RenderRecentSearch = () => {
    let recentSearch = JSON.parse(localStorage.getItem("RecentSearch")) || [];
    return (
      <div className="absolute flex flex-col top-[calc(100%+5px)] left-0 w-full bg-white shadow shadow-gray-300 rounded-lg p-1">
        <button className="px-1 md:px-3 py-2 flex justify-between hover:bg-gray-200 rounded-lg font-semibold">
          ...
          >
            {loadingRemove ? <Spinner /> : "Xóa"}
          </span>
        </button>
        {recentSearch.map((item, index) => (
          <button
            key={index}
            className="px-3 py-2 text-left hover:bg-gray-200 rounded-lg font-semibold"
            onClick={() => submitSearch(item, category)}
          >
            {item}
          </button>
        ))}
  ...
  return (
    <>
      <div
        ...
        className="text-gray-500 md:hidden flex hover:bg-gray-100 focus:outline-none focus:ring-4 focus:ring-gray-200 rounded-lg text-sm p-2.5"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"

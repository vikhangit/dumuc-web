        ...
        {items && items?.map(async (item, index) => {
              ...
              <div key={index} class={`flex justify-between items-center sm:gap-x-4 py-3 ${index !==0  ? "border-t" : "border-0"} border-gray-400`}>
                ...
                <div class="flex items-center gap-x-2">
                  ...
                  <div className="md:flex-1 md:min-w-0 text-sm font-medium text-gray-900">
                    <div className="text-sm sm:text-base font-medium text-[#363535]">{item?.name}</div>
                    <div className="text-xs sm:text-sm mt-1 text-[#747272] mb-3">Ngày tham gia: <span className=''>{moment(item?.createdAt)?.format('DD/MM/YYYY')}</span></div>
                    {user?.email ?
                    usingUser?.follows?.map(x => x?.authorId)?.includes(item?.authorId) ?
                    <button 
                        //unfollow
                        onClick={() => {
                            deleteUserFollow({
          ...
          setUsingUser(dataCall)
              ...
              <div className="flex flex-col justify-center">
                <button className="text-gray-700 text-[10px] sm:text-xs md:text-sm mt-2 text-center bg-white shadow shadow-gray-400 rounded-full py-0.5 px-1.5 sm:px-3 cursor-ponter relative z-50" onClick={async () => {
                  //getAuthorStatistics

...
const TableOfContent = ({ headers, onShowScroll}) => {
    ...
    return headers?.length !== 0 && (
        <div id="tab-content">
            <div className="mt-4 font-semibold text-sm"> Nội dung [{isHidden == false ? <a onClick={() => setIsHidden(true)}>Ẩn</a> : <a onClick={() => setIsHidden(false)}>Hiện</a>}]</div>
            {isHidden == false && 
                        ...
                        if (item.data?.level === 2) {
                            return (
                                <div key={index}>
                                    ...
                                    >
                                        <div dangerouslySetInnerHTML={{__html: item.data?.text}} />
                                    </Scroll.Link>
                                </div>
                            );
                            ...
                            return (
                                <div key={index}>
                                    <Scroll.Link
                                        ...
                                        offset={-90}
                                    >
                                        <svg class="w-3 h-3 mr-1 mt-1 text-gray-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 8 14">
                                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 13 5.7-5.326a.909.909 0 0 0 0-1.348L1 1"/>
                                        </svg> <div dangerouslySetInnerHTML={{__html: item.data?.text}} />
                                    </Scroll.Link>
                                </div>
                            );
                        }

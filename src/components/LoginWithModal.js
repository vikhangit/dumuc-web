...
const LoginWithModal = ({searchParams, setClose}) => {
    const query = searchParams;

    const [user] = useAuthState(auth)
    const [setUser, updating, error] = useUpdateProfile(auth);
    const router = useRouter();
    //const refId = useReferral();
    ...
    const loginGoogle = () => {
        ...
        .then(result => {
            ...
            createUser({
                ...
                //usernameReferral: refId ? parseInt(refId) : undefined,
            })
            .then(result => {
                setLoading(false);
    ...
    <div className="flex flex-col">
        ...
        <button 
            onClick={()=> loginGoogle()}
            type="button" 
            className={`flex items-center justify-center text-white bg-[#c80000] ${sizes.width > 350 ? "mx-10 sm:mx-20" : "mx-0"}  hover:brightness-110 focus:ring-4 focus:ring-blue-300 font-medium rounded-full text-sm px-5 py-2.5`}>

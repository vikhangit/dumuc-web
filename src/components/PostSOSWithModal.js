...
//apis
import {
  createUserRanking, generateCustomToken,
} from "@apis/users";
...
const PostSOSWithModal = ({ searchParams, setModalSuccess,setCloseForm}) => {
  ...
  const save = () => {
    ...
    if(user?.numberPlate === undefined || user?.numberPlate === ""){
      message.error("Vui lòng cập nhật biển số xe vào thông tin cá nhân của bạn")
      setLoading(false);
      return;
    }
  ...
  return (
    <main className="w-full">
      <div className="w-full sm:px-6 py-2" style={{ marginTop: 1 }}>
        ...
        justify-between  border-b border-[#CFC8C8] ${sizes.width > 380 ? "flex-row items-center pb-14" : "flex-col items-start pb-6"}`}>
          <div className={`flex flex-col gap-y-3 items-start ${sizes.width > 450 ? "w-[calc(100%-125px)] sm:w-[calc(100%-125px)]" : `${sizes.width > 380 ? "w-[calc(100%-80px)]" : "w-full"}`}`}>
            <div className={`flex w-full items-center justify-between`}>
              <div className={`${sizes.width > 380 ? "w-[80px] sm:w-[100px] text-xs sm:text-sm " : "w-[62px] text-[10px]"} font-semibold sm:font-medium`}>Tỉnh/ thành</div>
              <select disabled className={`${sizes.width > 380 ? "w-[calc(100%-85px)] sm:w-[calc(100%-105px)] text-xs sm:text-sm " : "w-[calc(100%-68px)] text-[10px]"} rounded-full font-semibold sm:font-medium  border border-[#CFC8C8]`}>
                <option>{address_components ? address_components[3]?.short_name : ""}</option>
              </select>
            </div>
            <div className="flex w-full items-center justify-between">
              <div className={`${sizes.width > 380 ? "w-[80px] sm:w-[100px] text-xs sm:text-sm " : "w-[62px] text-[10px]"} font-semibold sm:font-medium`}>Quận huyện</div>
              <select
              disabled
                className={`${sizes.width > 380 ? "w-[calc(100%-85px)] sm:w-[calc(100%-105px)] text-xs sm:text-sm " : "w-[calc(100%-68px)] text-[10px]"} rounded-full font-semibold sm:font-medium border border-[#CFC8C8]`}
              >
                <option>{address_components ? address_components[2]?.short_name : ""}</option>
              </select>
            </div>
            <div className="flex w-full items-center justify-between">
              <div className={`${sizes.width > 380 ? "w-[80px] sm:w-[100px] text-xs sm:text-sm " : "w-[62px] text-[10px]"} font-semibold sm:font-medium`}>Vị trí</div>
              <textarea
                className={`${sizes.width > 380 ? "w-[calc(100%-85px)] sm:w-[calc(100%-105px)] text-xs sm:text-sm " : "w-[calc(100%-68px)] text-[10px]"} rounded-lg font-semibold sm:font-medium  border border-[#CFC8C8]`}
                rows={4}
                readOnly
                value={`${address ? `${address_components[0]?.short_name} ${address_components[1]?.short_name}` : ""}`}
              ></textarea>
            </div>
          </div>
          <div className={`${sizes.width > 380 ? "w-[110px]" : "w-full"}  flex justify-end`}>
            <div
              className={`${sizes.width > 480 ? "w-[80px]" : `${sizes.width > 380 && sizes.width < 480 ? "w-[70px]" : "w-[calc(100%-68px)] ml-auto rounded-full mt-3"
                }`} font-semibold sm:font-medium bg-white shadow-md rounded-[100%] shadow-gray-500 ${sizes.width > 380 ? "text-[10px] sm:text-xs py-3" : "px-2 text-[10px] py-1"}  flex flex-col items-center justify-center text-center cursor-pointer`}
              onClick={() =>
                getMyLocation()
              }
            >
        ...
        justify-between ${sizes.width > 380 ? "flex-row items-center" : "flex-col items-start"}`}>
          <div className={`flex flex-col items-start ${sizes.width > 480 ? "w-[calc(100%-125px)] sm:w-[calc(100%-125px)]" : `${sizes.width > 380 ? "w-[calc(100%-105px)]" : "w-full"}`}`}>
            <div className={`flex w-full items-center justify-between`}>
              <div className={`${sizes.width > 380 ? "w-[80px] sm:w-[100px] text-xs sm:text-sm " : "w-[62px] text-[10px]"} font-semibold sm:font-medium`}>Chọn SOS</div>
              <select
                className={` border border-[#CFC8C8] ${sizes.width > 380
                  ? "w-[calc(100%-85px)] sm:w-[calc(100%-105px)] text-xs sm:text-sm "
                  : "w-[calc(100%-68px)] text-[10px]"} rounded-full font-semibold sm:font-medium`
                }
            ...
            {categoryError !== "" && (
              <div className={`flex w-full items-center`}>
                <div className={`${sizes.width > 380 ? "w-[80px] sm:w-[100px] text-xs sm:text-sm " : "w-[62px] text-[10px]"} font-semibold sm:font-medium`}></div>
            ...
            <div className="mt-3 flex w-full items-center justify-between">
              <div className={`${sizes.width > 380 ? "w-[80px] sm:w-[100px] text-xs sm:text-sm " : "w-[62px] text-[10px]"} font-semibold sm:font-medium`}>Số xe</div>
              <select
                className={
                  ` border border-[#CFC8C8] ${sizes.width > 380
                    ? "w-[calc(100%-85px)] sm:w-[calc(100%-105px)] text-xs sm:text-sm "
                    : "w-[calc(100%-68px)] text-[10px]"} rounded-full font-semibold sm:font-medium`
                }
            ...
            <div className="mt-3 flex w-full items-center justify-between">
              <div className={`${sizes.width > 380 ? "w-[80px] sm:w-[100px] text-xs sm:text-sm " : "w-[62px] text-[10px]"} font-semibold sm:font-medium`}>Thời hạn</div>
              <select
                className={
                  ` border border-[#CFC8C8] ${sizes.width > 380
                    ? "w-[calc(100%-85px)] sm:w-[calc(100%-105px)] text-xs sm:text-sm "
            ...
            {deadlineError !== "" && (
              <div className="flex w-full items-center">
              <div className={`${sizes.width > 380 ? "w-[80px] sm:w-[100px] text-xs sm:text-sm " : "w-[62px] text-[10px]"} font-semibold sm:font-medium`}></div>
              <p class="mt-1.5 ml-2 text-[10px] sm:text-xs text-[#c80000] font-semibold sm:font-medium" style={{
                color: "#c80000"
              }}>
                {deadlineError}
              </p>
            ...
            <div className="flex w-full items-center justify-between">
              <label
                for="message"
                className={`${sizes.width > 380 ? "w-[80px] sm:w-[100px] text-xs sm:text-sm " : "w-[62px] text-[10px] hidden"} font-semibold sm:font-medium`}
              >
                Ghi chú khác
              </label>
            </div>
          ...
          onClick={() => router.push("/account/profile")}
            className={`${sizes.width > 480 ? "w-[110px]" : `${sizes.width > 380 && sizes.width < 480 ? "w-[95px]" : "w-[calc(100%-68px)] ml-auto mt-3 block"
              }`} font-semibold sm:font-medium  border border-[#CFC8C8] px-2 border-dashed ${sizes.width > 380 ? "text-[10px] sm:text-xs py-3" : "px-2 text-[10px] py-1"}  flex flex-col items-start justify-center text-center cursor-pointer`}
          >
            <div>Người gửi</div>
        ...
        <label
                for="message"
                className={`${sizes.width > 380 ? "w-[80px] sm:w-[100px] text-xs sm:text-sm hidden" : "w-[62px] text-[10px] block"} font-semibold sm:font-medium`}
              >
                Ghi chú khác
              </label>
          <textarea
            id="message"
            rows="4"
            className={`${sizes.width > 380 ? "text-xs sm:text-sm mt-1.5" : "text-[10px] mt-1.5"} rounded-lg font-semibold sm:font-medium w-full py-2.5  border border-[#CFC8C8]`}
            placeholder=""
            onChange={(e) => {
        ...
        <div class="w-full mt-3">
          <div class="text-[10px] text-xs text-gray-900  border-b border-[#CFC8C8] font-semibold py-1 w-full">png, jpg</div>
          <div className="grid grid-cols-3 gap-x-2 sm:gap-x-4 gap-y-4 mt-4">
            ...
            })} */}
            {
              photos.map((item, index) => (
                <div className={`relative w-full ${sizes.width > 450 ? "h-32" : sizes.width > 376 ? "h-28" : sizes.width > 300 ? "h-24" : "h-20"}`}>
                  <input
                    type="file"
                    id={`photo${index}`}
                  ...
                  >
                    {
                      item != "" && item.length > 0 ? <Image src={item} alt="Image" width={0} height={0} sizes="100vw" className="rounded-md w-full h-full object-cover" /> : 
                    <div className="flex justify-center items-center w-full h-full">
                      {
                        loadingImage === index ? <Spinner /> :
          ...
          <div className={`grid  ${sizes.width > 420 ? "grid-cols-3 gap-x-4 items-center" : "grid-cols-2 gap-x-2" } mt-4 `}>
            <div className={`${sizes.width > 420 ? "col-span-1 h-fit py-4" : "h-32" } bg-white shadow-lg shadow-gray-500 flex flex-col justify-center items-center rounded-3xl cursor-pointer`}
            onClick={() => {
            ...
            >
              <Image src="/icons/play.png" width={0} height={0} sizes="100vw" className="w-6 h-6" />
              <div className="text-[10px] sm:text-xs font-medium text-[#c80000] mt-2">STREAM LIVE VIDEO</div>
            </div>
          ...
          {
            linkLiveStreamError != "" && 
            <div className={`grid  ${sizes.width > 420 ? "grid-cols-3 gap-x-4 items-center" : "grid-cols-2 gap-x-2" }`}>
            <div className={`${sizes.width > 420 ? "col-span-1 h-fit py-4" : "" } flex flex-col justify-center items-center rounded-3xl cursor-pointer`}
            >
            </div>
           
          <div  className={`${sizes.width > 420 ? "col-span-2" : "" }`}>
          <p className={`${sizes.width > 420 ? "mt-1.5" : "mt-3" } ml-2 text-[10px] sm:text-xs text-[#c80000] font-semibold sm:font-medium`}>
              {linkLiveStreamError}
            </p>
          </div>
          </div>
      ...
      <Modal show={showMap} onClose={() => setShowMap(false)} style={{
